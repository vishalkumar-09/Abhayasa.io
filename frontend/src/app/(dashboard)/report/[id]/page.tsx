      ...
      <div className="glass-card rounded-2xl p-6 border border-zinc-800/80 flex flex-col gap-5 relative overflow-hidden">
        ...
        <div className="flex items-center gap-2 border-b border-zinc-900 pb-3">
          ...
          <div>
            ...
            <p className="text-[10px] text-zinc-500 mt-0.5">Custom homework checklists compiled from your weaknesses</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-2.5">
              {[
                { id: "p1", text: `Review concepts matching: ${report.weaknesses && report.weaknesses.length > 0 ? report.weaknesses.map((w: string) => w.split(" ").slice(0,2).join(" ")).slice(0,2).join(", ") : "system design mechanisms"}` },
                { id: "p2", text: "Practice restructuring situational descriptions to strictly use the Situation ➡️ Task ➡️ Action ➡️ Result format." },
                { id: "p3", text: "Solve 2 Medium DSA problems based on tree structures or dynamic programming on a strict 25-minute timer." }
              ].map((step) => (
                <label key={step.id} className="flex items-start gap-3 p-3.5 rounded-xl bg-zinc-950/40 border border-zinc-900/60 hover:border-zinc-800 hover:bg-zinc-950/80 transition-all cursor-pointer select-none">
                  <input
                    type="checkbox"
                    className="mt-0.5 h-3.5 w-3.5 rounded border-zinc-800 text-violet-600 focus:ring-violet-500 bg-zinc-900 cursor-pointer accent-violet-600"
                  />
          ...
          <div className="flex flex-col gap-3.5">
            ...
            <div className="flex flex-col gap-2.5">
              {[
                { id: "g1", text: "Revise technical terms and complexity formulas (Time & Space O-notation) to state them clearly in explanations." },
                { id: "g2", text: "Update your target resume profile to highlight real-world projects involving databases and API structures." },
                { id: "g3", text: "Launch a new AI simulator session on HR mode to practice conflict-resolution behavioral patterns." }
              ].map((step) => (
                <label key={step.id} className="flex items-start gap-3 p-3.5 rounded-xl bg-zinc-950/40 border border-zinc-900/60 hover:border-zinc-800 hover:bg-zinc-950/80 transition-all cursor-pointer select-none">
                  <input
                    type="checkbox"
